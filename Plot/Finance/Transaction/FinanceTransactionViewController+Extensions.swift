        ...
        if taskList.indices.contains(taskIndex) {
            ...
            alert.addAction(UIAlertAction(title: "New Goal", style: .default, handler: { (_) in
                ...
                if let container = self.container {
                    self.showTaskDetailPush(task: nil, updateDiscoverDelegate: nil, delegate: self, event: nil, transaction: self.transaction, workout: nil, mindfulness: nil, template: nil, users: self.selectedFalconUsers, container: container, list: nil, startDateTime: nil, endDateTime: nil, isGoal: true)
                } else {
                    ...
                    self.showTaskDetailPush(task: nil, updateDiscoverDelegate: nil, delegate: self, event: nil, transaction: self.transaction, workout: nil, mindfulness: nil, template: nil, users: self.selectedFalconUsers, container: self.container, list: nil, startDateTime: nil, endDateTime: nil, isGoal: true)
                }
            }))
            alert.addAction(UIAlertAction(title: "New Task", style: .default, handler: { (_) in
                ...
                if let container = self.container {
                    self.showTaskDetailPush(task: nil, updateDiscoverDelegate: nil, delegate: self, event: nil, transaction: self.transaction, workout: nil, mindfulness: nil, template: nil, users: self.selectedFalconUsers, container: container, list: nil, startDateTime: nil, endDateTime: nil, isGoal: false)
                } else {
                    let containerID = Database.database().reference().child(containerEntity).childByAutoId().key ?? ""
                    self.container = Container(id: containerID, activityIDs: self.eventList.map({$0.activityID ?? ""}), taskIDs: self.taskList.map({$0.activityID ?? ""}), workoutIDs: self.healthList.filter({ $0.workout != nil }).map({$0.ID}), mindfulnessIDs: self.healthList.filter({ $0.mindfulness != nil }).map({$0.ID}), mealIDs: nil, transactionIDs: [self.transaction.guid], participantsIDs: self.transaction.participantsIDs)
                    self.showTaskDetailPush(task: nil, updateDiscoverDelegate: nil, delegate: self, event: nil, transaction: self.transaction, workout: nil, mindfulness: nil, template: nil, users: self.selectedFalconUsers, container: self.container, list: nil, startDateTime: nil, endDateTime: nil, isGoal: false)
                }
            }))
    ...
    func openEvent() {
        ...
        if eventList.indices.contains(eventIndex) {
            ...
            alert.addAction(UIAlertAction(title: "New Event", style: .default, handler: { (_) in
                ...
                if let container = self.container {
                    self.showEventDetailPush(event: nil, updateDiscoverDelegate: nil, delegate: self, task: nil, transaction: self.transaction, workout: nil, mindfulness: nil, template: nil, users: self.selectedFalconUsers, container: container, startDateTime: nil, endDateTime: nil)
                } else {
                    let containerID = Database.database().reference().child(containerEntity).childByAutoId().key ?? ""
                    self.container = Container(id: containerID, activityIDs: self.eventList.map({$0.activityID ?? ""}), taskIDs: self.taskList.map({$0.activityID ?? ""}), workoutIDs: self.healthList.filter({ $0.workout != nil }).map({$0.ID}), mindfulnessIDs: self.healthList.filter({ $0.mindfulness != nil }).map({$0.ID}), mealIDs: nil, transactionIDs: [self.transaction.guid], participantsIDs: self.transaction.participantsIDs)
                    self.showEventDetailPush(event: nil, updateDiscoverDelegate: nil, delegate: self, task: nil, transaction: self.transaction, workout: nil, mindfulness: nil, template: nil, users: self.selectedFalconUsers, container: self.container, startDateTime: nil, endDateTime: nil)
                }
            }))
