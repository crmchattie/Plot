...
//    }
    ...
    guard let currentUserID = Auth.auth().currentUser?.uid, let membersCount = conversation?.chatParticipantsIDs?.count, membersCount == 2 else { return }
        
        if let membersIDs = conversation?.chatParticipantsIDs, let index = membersIDs.firstIndex(of: currentUserID) {
        let toId = membersIDs[membersCount - index - 1]
    ...
    userHandler = userStatusReference.observe(.value, with: { (snapshot) in
      ...
      let value = snapshot.value as? NSDictionary
        ...
        if let name = value?["name"] as! String?, self.conversation?.chatName == name {
