...
open class ShoutView: UIView {
    ...
    public func setupFrames() {
        ...
        self.internalHeight = (window?.windowScene?.statusBarManager?.isStatusBarHidden ?? false ? 45 : 65)
        ...
        DispatchQueue.main.async {
            ...
            textOffsetY = window?.windowScene?.statusBarManager?.isStatusBarHidden ?? false ? 10 : 30
            if DeviceType.iPhoneX {
                textOffsetY = 10 + (window?.windowScene?.statusBarManager?.statusBarFrame.height ?? 0)
            }
    ...
    open override var frame: CGRect {
        didSet {
            if #available(iOS 11.0, *) {
                let window = UIApplication.shared.windows.filter {$0.isKeyWindow}.first
                if window?.windowScene?.statusBarManager?.isStatusBarHidden ?? false {
