        ...
        if taskList.indices.contains(taskIndex) {
            ...
            alert.addAction(UIAlertAction(title: "New Goal", style: .default, handler: { (_) in
                ...
                if let container = self.container {
                    self.showTaskDetailPush(task: nil, updateDiscoverDelegate: nil, delegate: self, event: nil, transaction: nil, workout: self.workout, mindfulness: nil, template: nil, users: self.selectedFalconUsers, container: container, list: nil, startDateTime: nil, endDateTime: nil, isGoal: true)
                } else {
                    ...
                    self.showTaskDetailPush(task: nil, updateDiscoverDelegate: nil, delegate: self, event: nil, transaction: nil, workout: self.workout, mindfulness: nil, template: nil, users: self.selectedFalconUsers, container: self.container, list: nil, startDateTime: nil, endDateTime: nil, isGoal: true)
                }
            }))
            alert.addAction(UIAlertAction(title: "New Task", style: .default, handler: { (_) in
                ...
                if let container = self.container {
                    self.showTaskDetailPush(task: nil, updateDiscoverDelegate: nil, delegate: self, event: nil, transaction: nil, workout: self.workout, mindfulness: nil, template: nil, users: self.selectedFalconUsers, container: container, list: nil, startDateTime: nil, endDateTime: nil, isGoal: false)
                } else {
                    let containerID = Database.database().reference().child(containerEntity).childByAutoId().key ?? ""
                    self.container = Container(id: containerID, activityIDs: self.eventList.map({$0.activityID ?? ""}), taskIDs: self.taskList.map({$0.activityID ?? ""}), workoutIDs: [self.workout.hkSampleID ?? ""], mindfulnessIDs: nil, mealIDs: nil, transactionIDs: self.purchaseList.map({$0.guid}), participantsIDs: self.workout.participantsIDs)
                    self.showTaskDetailPush(task: nil, updateDiscoverDelegate: nil, delegate: self, event: nil, transaction: nil, workout: self.workout, mindfulness: nil, template: nil, users: self.selectedFalconUsers, container: self.container, list: nil, startDateTime: nil, endDateTime: nil, isGoal: false)
                }
            }))
