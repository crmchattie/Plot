...
class ListDetailViewController: FormViewController {
    ...
    func setupVariables() {
        if let _ = list {
            ...
            list = ListType(id: ID, name: nil, color: CIColor(color: ChartColors.palette()[5]).stringRepresentation, source: ListSourceOptions.plot.name, admin: currentUser, defaultList: false, financeList: false, healthList: false, goalList: false)
        }
    }
