...
class GDeletePlotActivityOp: AsyncOperation {
    ...
    private func startRequest() {
        ...
        ParticipantsFetcher.getParticipants(forActivity: activity) { [weak self] users in
            ...
            activityAction.deleteActivity(updateExternal: false, updateDirectAssociation: false)
            self?.finish()
        }
    }
