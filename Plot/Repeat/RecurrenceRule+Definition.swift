...
public extension RecurrenceRule {
    ...
    func isWeekendRecurrence() -> Bool {
        guard frequency == .weekly && interval == 1 else {
            return false
        }
        let byweekday = self.byweekday.sorted(by: <)
        return byweekday == [.saturday, .sunday].sorted(by: <)
    }
    ...
    func isQuarterlyRecurrence(occurrence occurrenceDate: Date) -> Bool {
        guard frequency == .monthly && interval == 3 else {
            return false
        }
        guard bymonthday.count == 1 else {
            if bymonthday.count == 0 {
    ...
    func isSemiannualRecurrence(occurrence occurrenceDate: Date) -> Bool {
        guard frequency == .monthly && interval == 6 else {
            return false
        }
        guard bymonthday.count == 1 else {
            if bymonthday.count == 0 {
    ...
    func isCustomRecurrence(occurrence occurrenceDate: Date) -> Bool {
        return !isDailyRecurrence() &&
            ...
            !isQuarterlyRecurrence(occurrence: occurrenceDate) &&
            !isSemiannualRecurrence(occurrence: occurrenceDate) &&
    ...
    func typeOfRecurrence(language: RecurrencePickerLanguage = InternationalControl.shared.language, occurrence occurrenceDate: Date) -> String {
        ...
        } else if isWeekendRecurrence() {
            return internationalControl.localizedString("basicRecurrence.everyWeekend")
        } else if isQuarterlyRecurrence(occurrence: occurrenceDate) {
            return internationalControl.localizedString("basicRecurrence.everyThreeMonths")
        } else if isSemiannualRecurrence(occurrence: occurrenceDate) {
            return internationalControl.localizedString("basicRecurrence.everySixMonths")
