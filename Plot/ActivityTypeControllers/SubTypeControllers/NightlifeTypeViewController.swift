...
class NightlifeTypeViewController: ActivitySubTypeViewController, UISearchBarDelegate {
    ...
    var filters: [filter] = [.location, .fsNightlifeCategoryId]
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 1, repeats: false, block: { (_) in
            self.complexSearch(query: searchText.lowercased(), categories: self.filterDictionary["fsNightlifeCategoryId"] ?? [], lat: self.filterDictionary["lat"]?[0] ?? "", lon: self.filterDictionary["lon"]?[0] ?? "", favorites: self.filterDictionary["favorites"]?[0] ?? "")
        })
    }
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 1, repeats: false, block: { (_) in
            self.complexSearch(query: searchBar.text?.lowercased() ?? "", categories: self.filterDictionary["fsNightlifeCategoryId"] ?? [], lat: self.filterDictionary["lat"]?[0] ?? "", lon: self.filterDictionary["lon"]?[0] ?? "", favorites: self.filterDictionary["favorites"]?[0] ?? "")
        })
    }
...
extension NightlifeTypeViewController: UpdateFilter {
    func updateFilter(filterDictionary : [String: [String]]) {
        ...
        if !filterDictionary.values.isEmpty {
            ...
            complexSearch(query: "", categories: self.filterDictionary["fsNightlifeCategoryId"] ?? [], lat: self.filterDictionary["lat"]?[0] ?? "", lon: self.filterDictionary["lon"]?[0] ?? "", favorites: self.filterDictionary["favorites"]?[0] ?? "")
        } else {
            searchActivities = []
            self.filterDictionary = filterDictionary
