...
class FoodTypeViewController: ActivitySubTypeViewController, UISearchBarDelegate {
    ...
    var sections: [SectionType] = [.topFood,.americanFood, .seafoodFood, .vegetarianFood,  .cheapEats, .breakfastFood, .generalCoffee, .bakeryFood, .dessertFood, .fastFood, .asianFood, .italianFood, .mexicanFood, .middleeastFood]
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 1, repeats: false, block: { (_) in
            self.complexSearch(query: searchBar.text?.lowercased() ?? "", categories: self.filterDictionary["fsFoodCategoryId"] ?? [], lat: self.filterDictionary["lat"]?[0] ?? "", lon: self.filterDictionary["lon"]?[0] ?? "", favorites: self.filterDictionary["favorites"]?[0] ?? "")
        })
    }
